...
export function schedule(
  ...
  if(rating==='again'){
    ...
    intervalDays=rating==='good'?2:1;
    if(rating==='good')lapses=0;
  }else if(first){
    intervalDays=rating==='good'?3:1;
    if(rating==='good')lapses=0;
  }else{
...
export function buildTodayQueue(
  ...
  const wrong:Array<{id:string;due:number;lapses:number}>=[];
  const review:Array<{id:string;due:number}>=[];
  const fresh:string[]=[];
  ...
  for(const card of cards){
    ...
    else review.push({id:card.id,due:state.dueDay});
  }
  ...
  return[
    ...wrong.slice(0,wrongCap).map(x=>x.id),
    ...review.map(x=>x.id),
